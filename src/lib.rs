...
fn active_treatment<'a>(user_identifier: &str, feature: &'a Feature, context: &Context) -> &'a str {
    ...
    let overriding_treatment = find_overriding_treatment(user_identifier, segments, treatments_slice);
...
fn find_overriding_treatment<'a>(user_identifier: &str, segments: &Vec<Segment>, treatments_slice: &'a [Treatment]) -> Option<&'a String> {
        ...
        for treatment_segment in &treatment.segments {
            for segment in segments {
                if &segment.name == treatment_segment {
                    if segment.user_identifiers.contains(&user_identifier.to_string()) {
        ...
        let actual: Toggles = find_feature_toggles("pulp_fiction", Context {
            ...
            features: vec!(
                Feature {
                    ...
                    treatments: vec!(
                        Treatment { probability: 100, value: String::from("gold"), segments: vec!() }
                    ),
                }
            ),
            segments: vec!(),
        ...
        let actual: Toggles = find_feature_toggles("wouter", Context {
            ...
            features: vec!(
                Feature {
                    ...
                    treatments: vec!(
                        Treatment { probability: 100, value: String::from("gold"), segments: vec!() },
                        Treatment { probability: 0, value: String::from("silver"), segments: vec!() },
                    ),
                }
            ),
            segments: vec!(),
        ...
        let actual: Toggles = find_feature_toggles("wouter", Context {
            ...
            features: vec!(
                Feature {
                    ...
                    treatments: vec!(
                        Treatment { probability: 100, value: String::from("gold"), segments: vec!() },
                        Treatment { probability: 0, value: String::from("silver"), segments: vec!(String::from("beta-tester")) },
                    ),
                }
            ),
            segments: vec!(
